...
import { getHistory, getManga } from "../../redux/apiRequest";
import {
  Box,
  Card,
  CardMedia,
...
const History = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (historyData.metadata && historyData.metadata.length > 0) {
        const mangaPromises = historyData.metadata.map(
          async (history) => await getManga(history.history_manga_id, dispatch)
        );
